...
#  --> Process by mountain range:
# 1. creates a bbox around the mountain range
#    --> then, for each species (each row) in this mountain range:
# 2. check if the species range intersect with the bbox
# 3. calculate the overlap area in km2 and the % of overlap of the species range
# 4. removes all species with < 5km2 AND < 1% overlap with a GMBA Mountain range
...
# This snippet binds elevation data to species names (GARD)
...
#------------------------------------#
# 4.1. Set species dataframe -----
#------------------------------------#

# From the base dataframe with row = species X mountain range, we add their range distribution as a new column
...
# 5.3. Extract -----
#------------------------#
reptile_GBIF_elev <- extract.elevational.limits.GBIF(GBIF_clean, dem)

# A bit of cleaning
reptile_GBIF_elev <- reptile_GBIF_elev %>%
  rename(Mountain_range = "Level_03")

# Add GBIF elev to the base dataframe
reptile_dataframe <- reptile_dataframe %>%
  left_join(reptile_GBIF_elev, by = c("sciname", "Mountain_range"))
